...
int audio_CheckWavFmt()
{
  ...
  sprintf(wp_buf, "Channel = %d     ", audio_wav_fmt.wChannels);
  etft_DisplayString(wp_buf, 0, 80, 0xFFFF, 0); //显示声道数
  sprintf(wp_buf, "Samples = %dk     ", audio_wav_fmt.dwSamplesPerSec / 1000);
  etft_DisplayString(wp_buf, 0, 96, 0xFFFF, 0); //显示采样率
  sprintf(wp_buf, "Bits = %d     ", audio_wav_fmt.wBitsPerSample);
  etft_DisplayString(wp_buf, 0, 112, 0xFFFF, 0); //显示每个采样的比特数
...
{
  sprintf(wp_buf, "Time = %d:%d/%d:%d          ", cur / 60
          , cur % 60, all / 60, all % 60);
  etft_DisplayString(wp_buf, 0, 64, 0xFFFF, 0); //显示当前秒数
}

void wp_DisplayVolume()
{
  sprintf(wp_buf, "Volume = %d    ", curVolume);
  etft_DisplayString(wp_buf, 0, 128, 0xFFFF, 0); //显示当前音量
}
...
int app_PlayWavFile(const char* filename)
{
  ...
  etft_DisplayString("                    ", 0, 32, 0xFFFF, 0);
  etft_DisplayString(filename, 0, 32, 0xFFFF, 0);
  etft_DisplayString("   : Vol+         : Next        : Pause/", 0, 240 - 32, 0xFFFF, 0);
  etft_DisplayString("   : Vol-         : Exit        Continue", 0, 240 - 16, 0xFFFF, 0);
  etft_DisplayString("S4",0,240 - 32,etft_Color(255, 0, 0),0);
  etft_DisplayString("S3",120,240 - 32,etft_Color(255, 0, 0),0);
  etft_DisplayString("S5",230,240 - 32,etft_Color(255, 0, 0),0);
  etft_DisplayString("S6",0,240 - 16,etft_Color(255, 0, 0),0);
  etft_DisplayString("S7",120,240 - 16,etft_Color(255, 0, 0),0);
  ...
  while(curDataPtr < curChunkHeader.dwChunkSize)
  {
    while(decodedSampleCount > audio_OutputBufferSpace()) //此时等待缓冲区有空间再继续
    {
      ...
      if(rc == BTNRC_PAUSE)
      {
        etft_DisplayString("PAUSE", 0, 48, etft_Color(255,0,0), 0);
        while(wp_ProcessBtns() != BTNRC_PAUSE);
        etft_DisplayString("     ", 0, 48, etft_Color(255,0,0), 0);
      }
    }
